...
OUT_DIR="${OUT_DIR:-/home/vanja/hassok8s/logs/tests/baseline-token-$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$OUT_DIR"
...
for i in $(seq 1 "$COUNT"); do
  TS="$(date -Iseconds)"
  BODY_FILE="$OUT_DIR/body-${i}.json"

  LINE="$(curl -k -s -o "$BODY_FILE" \
    ...
    -d "username=$USERNAME" \
    -d "password=$PASSWORD" || echo "000,0")"
